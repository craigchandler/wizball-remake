...
void PARSER_put_values_of_word_or_words(int word_list_index, char *word, temp_script_data *script_element)
{
	...
	else
	{
		...
		if ((STRING_instr_char(word_pointer, '-', 0) != UNSET) && (word_list_index != math_list_index))
		{
			...
			{
				snprintf(error, sizeof(error), "ILLEGAL NEGATION OF WORD '%s' OF FROM WORD LIST '%s' IN SCRIPT '%s'", word_pointer, word_list[word_list_index].name, error_current_script);
		...
		if ((word_pointer[0] == '!') && (word_list_index != compare_list_index) && (word_list_index != math_list_index) && (word_list_index != math_compare_list_index))
		{
			...
			{
				snprintf(error, sizeof(error), "ILLEGAL INVERSION OF WORD '%s' OF FROM WORD LIST '%s' IN SCRIPT '%s'", word_pointer, word_list[word_list_index].name, error_current_script);
				OUTPUT_message(error);
				assert(0);
			}
		}
...
bool PARSER_do_word_or_words_exist_in_list(int word_list_index, char *word, int *index_1, int *index_2)
{
	...
	if (STRING_instr_char(word_pointer, '.', 0) != -1) // Any dots in the middle? It's a word_1.word_2 one! Yay!
	{
		...
		if (word_2 == NULL)
		{
			snprintf(error, sizeof(error), "WORD1.WORD2 missing WORD2 '%s' IN SCRIPT '%s'", word_backup, error_current_script);
			OUTPUT_message(error);
			assert(0);
		}
	...
	{
		snprintf(error, sizeof(error), "ILLEGAL NEGATION OF WORD '%s' OF FROM WORD LIST '%s'", word_pointer, word_list[word_list_index].name);
		OUTPUT_message(error);
		assert(0);
	}
	...
	{
		snprintf(error, sizeof(error), "ILLEGAL INVERSION OF WORD '%s' OF FROM WORD LIST '%s'", word_pointer, word_list[word_list_index].name);
		OUTPUT_message(error);
		assert(0);
	}
...
void PARSER_compiler_directive(char *line)
{
	...
	if (strcmp(pointer, "#DEFINE") == 0)
	{
		...
		if (pointer == NULL)
		{
			snprintf(error, sizeof(error), "#DEFINE MISSING 2 ARGUMENTS IN SCRIPT '%s'", error_current_script);
			OUTPUT_message(error);
			assert(0);
		}
		...
		{
			{
				int _n = snprintf(error, sizeof(error), "#DEFINE MISSING 1 ARGUMENT AFTER '%s' IN SCRIPT '", line_backup);
				if (_n < 0)
					_n = 0;
				size_t _rem = (_n < (int)sizeof(error)) ? sizeof(error) - _n - 1 : 0;
				if (_rem > 0)
				{
					strncat(error, error_current_script, _rem);
				}
				strncat(error, "'", sizeof(error) - strlen(error) - 1);
			}
			OUTPUT_message(error);
			assert(0);
		}
...
void PARSER_read_script(char *filename, bool parsing_actual_script = false, bool remove_commas_and_brackets = false)
{
	...
	{
		snprintf(error, sizeof(error), "ERROR! SCRIPT '%s' NOT FOUND FOR LOADING! YOU FLIPPIN' MUPPET!", filename);
		OUTPUT_message(error);
		assert(0);
	}
...
int PARSER_parse_list(int line_number, int data_source, bool autonumber = false, bool exponential_numbering = false)
{
	...
	if ((data_source == SOURCE_WORD_LIST) || (data_source == SOURCE_INDEX_LIST))
	{
		...
		{
			// Fook!
			snprintf(error, sizeof(error), "ERROR! WORD LIST '%s' NOT FOUND FOR INDEX/FILE LISTING!", script[line_number].text_word_list[3].text_word);
			OUTPUT_message(error);
		}
	}
	...
	case SOURCE_FILE: // -------- READING LIST FROM FILE --------
		...
		{
			snprintf(error, sizeof(error), "ERROR! FILE '%s' NOT FOUND FOR LISTING!", filename);
			OUTPUT_message(error);
		}
...
int PARSER_process_script(void)
{
	...
	for (line_number = 0; line_number < temp_tokenised_script_length; line_number++)
	{
		...
		if (stack_pointer < 0)
		{
			snprintf(error, sizeof(error), "STACK POINTER MISMATCH IN SCRIPT '%s'", error_current_script);
			OUTPUT_message(error);
			assert(0); // Aw... crap!
		}
	...
	for (line_number = 0; line_number < temp_tokenised_script_length; line_number++)
	{
		...
		if (temp_tokenised_script[line_number].indentation_data & STORE_EQUAL_INDENT_LINE)
		{
			do
			{
				...
				{
					snprintf(error, sizeof(error), "COULD NOT FIND MATCHING INDENTATION IN SCRIPT '%s'", error_current_script);
					OUTPUT_message(error);
			...
			snprintf(number_as_string, sizeof(number_as_string), "%d", other_line_number);
		...
		else if (temp_tokenised_script[line_number].indentation_data & STORE_LESSER_INDENT_LINE)
		{
			do
			{
				...
				if (other_line_number == temp_tokenised_script_length)
				{
					snprintf(error, sizeof(error), "COULD NOT FIND LESSER INDENTATION IN SCRIPT '%s'", error_current_script);
					OUTPUT_message(error);
					assert(0); // FOOOOOOOOOK! What happened there then?
				}
		...
		else if (temp_tokenised_script[line_number].indentation_data & STORE_PREV_EQUAL_INDENT_LINE)
		{
			do
			{
				...
				{
					snprintf(error, sizeof(error), "COULD NOT FIND PREVIOUS MATCHING INDENTATION IN SCRIPT '%s'", error_current_script);
					OUTPUT_message(error);
					assert(0); // FOOOOOOOOOK! What happened there then?
				}
			} while (temp_tokenised_script[line_number].indentation_level != temp_tokenised_script[other_line_number].indentation_level);
...
void PARSER_output_processed_script_back_as_text(int script_number)
{
	...
	if (file_pointer == NULL)
	{
		snprintf(error, sizeof(error), "CANNOT OUTPUT SCRIPT '%s' TO PROCESSED SCRIPT FILE", error_current_script);
		OUTPUT_message(error);
		assert(0); // Arse!
	}

	snprintf(tokenised_line_text, sizeof(tokenised_line_text), "%s\n\n", word_list[script_list_index].words[script_number].text_word);
	...
	for (line_number = 0; line_number < temp_tokenised_script_length; line_number++)
	{
		...
		snprintf(tokenised_line_text, sizeof(tokenised_line_text), "(%5d) %3d:\t", cumulative_line, line_number);
		...
		for (argument_number = 0; argument_number < temp_tokenised_script[line_number].script_line_size; argument_number++)
		{
			...
			if (source_word_list == label_list_index)
			{
				...
				snprintf(word, sizeof(word), "%s", word_list[label_list_index].words[source_word_list_entry_1].text_word);
			...
			{
				// It's a label, so just make a number and stick an L in front of it.
				snprintf(word, sizeof(word), "(ELSE GOTO LINE %d)", source_word_list_entry_1 + 1);
			...
			else if (source_word_list == number_list_index)
			{
				...
				{
					snprintf(word, sizeof(word), "%s", PARSER_get_alias_by_index(temp_tokenised_script[line_number].script_line_pointer[argument_number].alias_1_index));
				}
				else
				{
					snprintf(word, sizeof(word), "%d", source_word_list_entry_1);
			...
			else
			{
				...
				if (source_word_list_entry_2 != UNSET)
				{
					...
					if (temp_tokenised_script[line_number].script_line_pointer[argument_number].alias_1_index != UNSET)
					{
						...
						{
							snprintf(word, sizeof(word), "[%s]%s.[%s]%s", word_list[source_word_list].words[source_word_list_entry_1].text_word, PARSER_get_alias_by_index(temp_tokenised_script[line_number].script_line_pointer[argument_number].alias_1_index), word_list[source_word_list].words[source_word_list_entry_2].text_word, PARSER_get_alias_by_index(temp_tokenised_script[line_number].script_line_pointer[argument_number].alias_2_index));
						}
						else
						{
							snprintf(word, sizeof(word), "[%s]%s.%s", word_list[source_word_list].words[source_word_list_entry_1].text_word, PARSER_get_alias_by_index(temp_tokenised_script[line_number].script_line_pointer[argument_number].alias_1_index), word_list[source_word_list].words[source_word_list_entry_2].text_word);
						}
					}
					else
					{
						if (temp_tokenised_script[line_number].script_line_pointer[argument_number].alias_2_index != UNSET)
						{
							snprintf(word, sizeof(word), "%s.[%s]%s", word_list[source_word_list].words[source_word_list_entry_1].text_word, word_list[source_word_list].words[source_word_list_entry_2].text_word, PARSER_get_alias_by_index(temp_tokenised_script[line_number].script_line_pointer[argument_number].alias_2_index));
						}
						else
						{
							snprintf(word, sizeof(word), "%s.%s", word_list[source_word_list].words[source_word_list_entry_1].text_word, word_list[source_word_list].words[source_word_list_entry_2].text_word);
						}
					}
				}
				else
				{
					...
					if (temp_tokenised_script[line_number].script_line_pointer[argument_number].alias_1_index != UNSET)
					{
						snprintf(word, sizeof(word), "[%s]%s", word_list[source_word_list].words[source_word_list_entry_1].text_word, PARSER_get_alias_by_index(temp_tokenised_script[line_number].script_line_pointer[argument_number].alias_1_index));
					}
					else
					{
						snprintf(word, sizeof(word), "%s", word_list[source_word_list].words[source_word_list_entry_1].text_word);
					}
				}
				skip_this_word_for_tracer_script = false;
			}
		...
		for (argument_number = 0; argument_number < temp_tokenised_script[line_number].script_line_size; argument_number++)
		{
			snprintf(word, sizeof(word), "(B%i : T%i : V%i)", temp_tokenised_script[line_number].script_line_pointer[argument_number].data_bitmask, temp_tokenised_script[line_number].script_line_pointer[argument_number].data_type, temp_tokenised_script[line_number].script_line_pointer[argument_number].data_value);
	...
	snprintf(tokenised_line_text, sizeof(tokenised_line_text), "\n\n\n");
	fputs(tokenised_line_text, file_pointer);
...
void PARSER_add_to_global_parameter_list(int line_number)
{
	...
	if (file_pointer != NULL)
	{
		...
		*/
		snprintf(temp_char, sizeof(temp_char), "#PARAMETER LIST SIZE = %d\n", list_size);
		fputs(temp_char, file_pointer);
...
int PARSER_evaluate_data_argument(char *file_text_line, char *pointer_2, int unused_value)
{
	...
	char word_copy[TEXTFILE_SUPER_SIZE];
	char error[TEXTFILE_SUPER_SIZE]; // If we're using a text file we might need to burp out an error if it ain't there...
	char grabbed_word[TEXTFILE_SUPER_SIZE];
	...
	if (pointer_2 != NULL)
	{
		...
		if ((colon_position = STRING_instr_char(pointer_2, ':', 0)) != UNSET)
		{
			...
			{
				{
					int _n = snprintf(error, sizeof(error), "Unrecognised data list name '%s' in line '", list_name);
					if (_n < 0)
						_n = 0;
					size_t _rem = (_n < (int)sizeof(error)) ? sizeof(error) - _n - 1 : 0;
					if (_rem > 0)
					{
						strncat(error, file_text_line, _rem);
					}
					strncat(error, "'", sizeof(error) - strlen(error) - 1);
				}
				OUTPUT_message(error);
			...
			else
			{
				if (STRING_instr_char(list_entry, '|', 0) != UNSET)
				{
					...
					do
					{
						...
						{
							{
								int _n = snprintf(error, sizeof(error), "Unrecognised data list entry '%s' in line '", grabbed_word);
								if (_n < 0)
									_n = 0;
								size_t _rem = (_n < (int)sizeof(error)) ? sizeof(error) - _n - 1 : 0;
								if (_rem > 0)
				...
				else
				{
					...
					{
						{
							int _n = snprintf(error, sizeof(error), "Unrecognised data list entry '%s' in line '", list_entry);
							if (_n < 0)
								_n = 0;
							size_t _rem = (_n < (int)sizeof(error)) ? sizeof(error) - _n - 1 : 0;
							if (_rem > 0)
		...
		else
		{
			...
			int _n = snprintf(error, sizeof(error), "Unrecognised data entry '%s' in line '", pointer_2);
			size_t cur = strlen(error);
			size_t _rem = (sizeof(error) > cur) ? (sizeof(error) - cur - 1) : 0;
			if (_rem > 0)
			{
				strncat(error, file_text_line, _rem);
			}
			/* ensure trailing quote */
			if (sizeof(error) > strlen(error) + 1)
			{
				strncat(error, "'", sizeof(error) - strlen(error) - 1);
			}
			OUTPUT_message(error);
...
bool PARSER_parse(char *filename)
{
	...
	char compiled_script_filename[TEXT_LINE_SIZE];
	...
	char script_filename[TEXT_LINE_SIZE];
	...
	{
		snprintf(script_filename, sizeof(script_filename), "%s\\%s%s", word_list[script_list_index].name, word_list[script_list_index].words[script_number].text_word, word_list[script_list_index].extension);
